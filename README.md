...
More in `docs/screenshots/`, including the crawler's content-type handling and the XSS-check verification run.
Installation
```bash
...
```
Full setup (including test target setup) is in the User Guide.
Usage
CLI
```bash
...
Both targets were scanned at default settings. Full reports: `docs/sample_report_juiceshop.md`.
...
The difference is real and explainable: Juice Shop suppresses more fingerprinting headers but leaks server errors; DVWA is missing more baseline security headers but doesn't disclose server errors. See the User Guide for how scoring works.
Known Limitations
